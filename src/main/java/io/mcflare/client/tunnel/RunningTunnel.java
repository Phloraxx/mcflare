...
/** Minecraft-facing wrapper around the version-independent MCflare carrier. */
public final class RunningTunnel implements AutoCloseable {
    ...
    public static @NotNull RunningTunnel create(String host, LoopbackCarrier.ErrorHandler errorHandler)
            throws IOException {
        LoopbackCarrier carrier = LoopbackCarrier.start(host, PROBE_PATH, errorHandler);
        return new RunningTunnel(new Access(host, carrier.getLocalAddress()), carrier);
    }

    public Access access() { return access; }
    @Override public void close() { carrier.close(); }
