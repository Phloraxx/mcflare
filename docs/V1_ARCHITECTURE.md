...
Authenticated Mojang `online-mode=true` login is proven through both supported Cloudflare delivery modes with the exact `1.0.1` release-package Fabric client. IANA registration, a naturally occurring Cloudflare-edge-initiated WebSocket termination, and larger graphical/world-generation stress remain optional future validation/formalization for this hobby project.
