...
- Both real joins restored `<redacted-public-ip>` into Minecraft's login log, matching the Oracle client host public IPv4.
...
## Release status and optional validation

Real rebuilt Fabric 26.1 player login/world join through `/mcflare` is proven on both true Orange and named Tunnel, including restored IP visibility in Minecraft login logs. Minecraft's native `ban-ip` also acts on that restored address and rejects a fresh Cloudflare-routed reconnect. The same MCflare-equipped client joins an ordinary zero-MCflare server through normal TCP, and the actual graphical Multiplayer server-list pinger successfully resolves and renders that ordinary server. A real external IPv6 client has proven true-Orange visitor-IP restoration as `PROXY TCP6` and a live Status response through the Fabric parser. Public WSS Status concurrency is characterized to 128 simultaneous connections per delivery path with zero failures. Higher-scale full Minecraft protocol concurrency is also proven beyond the three graphical clients: 16 simultaneous GAME-state clients held 45 seconds on each delivery mode, followed by four 16-client churn cohorts per mode (64/64 additional joins each). Authenticated online-mode proof, IANA registration/public protocol publication, and observation of a genuine Cloudflare-edge-initiated WebSocket termination are optional future validation/formalization, not hobby-release blockers. Larger graphical/world-generation stress is optional backend/performance characterization, not an unproven MCflare transport gate. The fresh-chunk teleport-burst, public-IPv6, local connector-restart, client-network black-hole, higher-scale transport/session, and loader/version gates are complete.
