...
Real visitor IP is now a v1 requirement. Cloudflare's `CF-Connecting-IP` / `CF-Connecting-IPv6` is translated by the gateway into standard HAProxy PROXY v1. Fabric and NeoForge share a bounded local-source-trusted PROXY-v1 parser that updates `Connection.address`; loopback is trusted by default, with only the exact verified local `server-ip` address added while the integrated gateway is active; no separate Netty HAProxy codec is bundled. The Fabric artifacts are also runtime-proven unchanged on Quilt. Paper/Purpur use one Java-21 server plugin and their native PROXY implementation instead of Minecraft networking injection.
